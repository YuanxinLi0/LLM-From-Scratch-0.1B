"""
LLM-From-Scratch-0.1B 模型交互式对话脚本（简化版）
"""
import argparse
import os
import torch
...
def main():
    ...
    parser = argparse.ArgumentParser(description="LLM-From-Scratch-0.1B 模型交互对话")
    parser.add_argument('--model_path', default=os.path.join(project_root, 'checkpoints', 'sft_768.pth'), type=str, help="模型权重路径（.pth 文件）")
    parser.add_argument('--tokenizer_path', default=os.path.join(project_root, 'tokenizer_15k'), type=str, help="Tokenizer 路径")
    ...
    parser.add_argument('--top_p', default=0.7, type=float, help="nucleus 采样阈值")
    ...
    model = LLMFromScratchForCausalLM(LLMFromScratchConfig(
    ...
    while True:
        ...
        # 生成回复
        print('🤖 LLM-From-Scratch-0.1B: ', end='', flush=True)
        with torch.no_grad():
